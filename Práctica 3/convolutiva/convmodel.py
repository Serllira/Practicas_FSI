...
example_batch_train, label_batch_train = dataSource(["data3/001/*.jpg", "data3/010/*.jpg", "data3/100/*.jpg"], batch_size=batch_size)
example_batch_valid, label_batch_valid = dataSource(["data3/001validacion/*.jpg", "data3/010validacion/*.jpg", "data3/100validacion/*.jpg"], batch_size=batch_size)
example_batch_test, label_batch_test = dataSource(["data3/001test/*.jpg", "data3/010test/*.jpg", "data3/100test/*.jpg"], batch_size=batch_size)
...
with tf.Session() as sess:
    ...
    erroresValidacion=[]
    epoch=0
    errorAnteriorValidacion = 0
    errorActualValidacion = 1
    estabilidad = 0

    while estabilidad < 7 and errorActualValidacion > 0.003:
        sess.run(optimizer)
        errorActualValidacion = sess.run(cost_valid)
        erroresValidacion.append(errorActualValidacion)

        errorActualEntrenamiento = sess.run(cost)
        #print("Error Entrenamiento: ", errorActualEntrenamiento)
        erroresEntrenamiento.append(errorActualEntrenamiento)

        print("Iter: ", epoch, " | Error Validacion: ", errorActualValidacion, " | Error Anterior Validacion: ", errorAnteriorValidacion,
               " | Estabilidad: ", estabilidad)

        if (errorActualValidacion >= errorAnteriorValidacion * 0.9):
            estabilidad += 1
        else:
             estabilidad = 0

        epoch += 1
        errorAnteriorValidacion = errorActualValidacion
